...
def initial_operation():
    ...
    mortality_rate = df.loc[(df["Country Name"].isin(countries))& 
    (df["Series Name"] == "Mortality rate, under-5 (per 1,000 live births)"),
    year]
...
def generate_linechart(mortality_rate):
    ...
    mortality_rate_t = mortality_rate_type_conversion(mortality_rate)
...
def generate_piechart(mortality_rate):
...
def generate_barchart():
    ...
    plt.bar(x, df_2["Clean energy"],  
	width = width, color = 'r',  
	label = 'Clean energy', edgecolor = 'black') 
    plt.bar(x + width, df_2["Fossil fuels"],
	width = width,  color = 'y', 
...
mortality_rate=initial_operation()

# to generate linechart
generate_linechart(mortality_rate)

# to generate pie chart
generate_piechart(mortality_rate)
